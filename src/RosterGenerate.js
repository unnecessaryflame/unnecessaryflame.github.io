...
function RosterGenerate (props) {
    return Object.entries(props).map((i, key) => (
            <Grid item lg={4} md={12} xs={12} key={key}>
                <Card style={Cards}>
                    {/*<CardActionArea href={'https://www.warcraftlogs.com/character/id/' + i[1].wcLogsID}>*/}
                <CardMedia image={process.env.PUBLIC_URL + '/emblems/'+i[1].class + '.png'} style={cardImage}>
                    <CardContent>
                        <Typography variant="h4" align='left' className={i[1].class} style={Text}>{i[1].name}</Typography>
